...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: '키워드 목록 조회 실패' }, { status: 500 });
  }
}

export async function POST(request: NextRequest) {
  try {
    const { topic, primaryKeywords = [] } = await request.json();
    ...
    return NextResponse.json({ error: '키워드 리서치 실패' }, { status: 500 });
  }
}
